  ...
  "inferred": {
    ...
    "rbudget:correction": {
      ...
      "properties": {
        ...
        "learned_from": "agent run across the September statement"
      }
    }
  },
...
| `inferred` | Models, via push operations; clients holding `write:inferred` | **A map keyed by writer and task** — the store's analyses under its identifier (`rhizome:categorize`) and a client's under its registered name (`rbudget:forecast`). Callers provide a bare task name; the store assigns the authenticated writer prefix. Writers are globally unique within a store, so keys cannot collide and there is no unprefixed case. People hold no writer namespace: a person's assertions and corrections are `user` block data, not inference. Each entry carries its own `model`, `inferred_at`, optional `confidence`, and `properties`, so the metadata describes exactly one inference. **This block is memory scoped to the record.** Some entries are task output, recomputed from `source` whenever the task re-runs. Others accumulated — a pattern an agent noticed across several objects, an understanding built over a conversation — and re-running a task cannot reproduce them. Entries of the second kind set `durable: true`, and a push task MUST NOT replace a durable entry. A task can never set the flag on its own output — `durable` means *cannot be reproduced by re-running*, and task output is by definition what re-running produces — so only agent runs may set it. Otherwise a re-run replaces only its own key. Consumers MUST treat entries as advisory. |
...
**`tweet`** — `published_at` and `post_kind` are required; author handle/name, conversation and referenced-post IDs, language, sensitivity/edit metadata, and structured entities are optional source facts. Keys include `x_tweet_id`, `x_author_id`, and the canonical post URL. Exact post text is the first `text/plain` element and is not duplicated into properties; image and video elements follow in source order. Quote targets remain inert identifiers or URLs rather than object relationships. Volatile engagement metrics do not participate in semantic identity.

**`fitness_activity`** — one recorded physical exercise session, independent of provider. Required: `sport` and at least one start representation: `started_at` for a known RFC3339 instant, or `started_local` for a calendar-valid local timestamp without a zone (`YYYY-MM-DDTHH:mm:ss[.fraction]`). Producers MUST NOT invent an offset for an unzoned source time. RFC3339's `-00:00` represents a known UTC instant whose local offset is unknown; it MUST NOT be used to turn an unzoned wall-clock reading into UTC. If both start representations appear, they MUST describe the same start. `timezone` is optional source evidence, preferably an IANA name. `run`, `ride`, `walk`, `swim`, and `workout` are canonical spellings for those sports; other nonempty sport names are legal.

Optional measurements use meters (`distance_m`, `elevation_gain_m`) and seconds (`elapsed_time_s`, `timer_time_s`, `moving_time_s`). Missing measurements MUST be omitted rather than represented by zero or null. Elapsed time includes pauses; timer time excludes recorded timer pauses; moving time requires source movement evidence. Timestamp differences alone MUST NOT be called moving or timer time. `title` and an explicit source `workout_type` (for example `race`) are facts; a title alone does not establish race classification. Owner-entered official results belong in `user.properties`, and model interpretations belong in `inferred`.

`laps` are recorded laps with one-based `index`, available `distance_m` and/or `duration_s`, required `timing_basis` and `provenance`, and optional `distance_basis`. They retain their recorded distances and MUST NOT be relabeled as mile splits. `splits` are calculated distance segments with required `index`, nominal `target_distance_m`, actual `distance_m`, `duration_s`, `timing_basis`, `distance_basis`, `method`, and `provenance`. Timing basis is `elapsed`, `timer`, or `moving`; distance basis is `recorded` or `gps`. `provenance` identifies the source format or measurement origin. `method` identifies the deterministic calculation (for example `linear_interpolation`); producers MUST document its boundary/interpolation rule. A final partial split retains its actual distance. Splits MUST derive from distance/time evidence rather than duplicating total average pace, and do not assert equality with a provider's processing or official race measurements. Both arrays are ordered and limited to 1,000 closed records each; raw sensor streams remain in the origin. Producers MUST omit unavailable split evidence rather than fabricate it.

Activity properties stay open for additional source facts. Source identifiers belong in `keys` (for example `strava_activity_id`); no provider identifier is required by the vocabulary. Normally zero elements: measurements are queried fields, not a new media kind. Authored prose and photographs, when present, use existing element kinds.
