...
const minimal: ActivityProperties = { sport: "run", started_local: "2026-09-12T06:30:00" };
const split = {
  index: 1,
...
describe("registered activity vocabulary", () => {
  test("uses generated public types and validators with both start representations", () => {
    const instant: ActivityProperties = { sport: "walk", started_at: "2026-09-12T06:30:00-04:00" };
    expect(validators.activity(minimal)).toBe(true);
    expect(validateSchema("activity", instant).ok).toBe(true);
    expect(validateMediaObject(object(minimal)).ok).toBe(true);
    expect(validateMediaObjectProperties("activity", { sport: "run" }).ok).toBe(false);
    expect(validateMediaObjectProperties("activity", { started_at: instant.started_at }).ok).toBe(false);
  });

  test("preserves unknown metrics and open source extensions for non-Strava activities", () => {
    const activity: ActivityProperties = {
      ...minimal,
      sport: "workout",
    ...
    expect(validateSchema("activity", activity).ok).toBe(true);
    expect(validateMediaObject(object(activity)).ok).toBe(true);
    expect(validateSchema("activity", { ...activity, distance_m: null }).ok).toBe(false);
  });
  ...
  test.each(["distance_m", "elapsed_time_s", "timer_time_s", "moving_time_s", "elevation_gain_m"])(
    ...
    (field) => {
      for (const value of [-1, "10", Number.NaN, Number.POSITIVE_INFINITY]) {
        expect(validateSchema("activity", { ...minimal, [field]: value }).ok).toBe(false);
      }
    },
  );
  ...
  test("keeps recorded laps distinct from computed full and partial distance splits", () => {
    ...
    expect(validateSchema("activity", activity).ok).toBe(true);
    expect(validateSchema("activity", { ...minimal, laps: [{ index: 1, duration_s: 20, timing_basis: "timer", provenance: "manual" }] }).ok).toBe(true);
    expect(validateSchema("activity", { ...minimal, laps: [{ index: 1, timing_basis: "timer", provenance: "fit" }] }).ok).toBe(false);
  });
  ...
  test.each([
    ...
    expect(validateSchema("activity", { ...minimal, splits: [{ ...split, ...invalid }] }).ok).toBe(false);
  });
  ...
  test("requires calculation evidence, closes nested records, and bounds arrays", () => {
    for (const field of ["duration_s", "timing_basis", "distance_basis", "method", "provenance"]) {
      ...
      expect(validateSchema("activity", { ...minimal, splits: [incomplete] }).ok).toBe(false);
    }
    expect(validateSchema("activity", { ...minimal, laps: [{ index: 1, distance_m: 1000, duration_s: 300, timing_basis: "elapsed", provenance: "tcx", method: "invented" }] }).ok).toBe(false);
    expect(validateSchema("activity", { ...minimal, splits: Array.from({ length: 1001 }, (_, i) => ({ ...split, index: i + 1 })) }).ok).toBe(false);
    expect(validateSchema("activity", { ...minimal, laps: Array.from({ length: 1001 }, (_, i) => ({ index: i + 1, duration_s: 1, timing_basis: "timer", provenance: "fit" })) }).ok).toBe(false);
  });
