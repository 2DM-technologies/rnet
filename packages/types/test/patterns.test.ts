...
import { RNET_RECORD_KINDS, UUIDV7, rnetUriPattern, type RnetRecordKind } from "../src/patterns.ts";
...
const schemaPatterns = [...new Set(rnetSchemas.flatMap((schema) => collectPatterns(schema)))];
const uuidBearing = schemaPatterns.filter((pattern) => pattern.includes(UUIDV7));
...
describe("record identity patterns", () => {
  test("every UUID-bearing schema pattern is reproducible from rnetUriPattern", () => {
    ...
    for (const kind of RNET_RECORD_KINDS) reproducible.add(rnetUriPattern(kind));
  ...
  test("the canonical schemas actually carry UUIDv7 URIs", () => {
    ...
    for (const kind of ["element", "object", "origin", "vibe"] as RnetRecordKind[]) {
      expect(uuidBearing).toContain(rnetUriPattern(kind));
    }
  });

  test("owner URIs are opaque, not UUIDs", () => {
    const owner = schemaPatterns.find((pattern) => pattern.startsWith("^rnet://id/"));
  ...
  test("rnetUriPattern matches real URIs and rejects UUIDv4", () => {
    ...
    expect(new RegExp(rnetUriPattern("object")).test(`rnet://object/${v7}`)).toBe(true);
    expect(new RegExp(rnetUriPattern("object")).test(`rnet://object/${v4}`)).toBe(false);
    expect(new RegExp(rnetUriPattern("object")).test(`rnet://vibe/${v7}`)).toBe(false);
    expect(new RegExp(rnetUriPattern("origin", "client")).test(`rnet://client/${v7}`)).toBe(true);
  });
